select summit_id, min(valid_on) as valid_on, max(grade_level) as grade_level, count(crs_id) as cnt 
from (
  select summit_id, grade_level, student_id, crses.valid_on, crses.id as crs_id 
...
group by summit_id 
order by valid_on, cnt DESC, summit_id;
